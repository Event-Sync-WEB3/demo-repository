      ...
      {favorites.length === 0 ? (
        ...
        <div className="flex flex-col gap-2">
          {favorites.map((id) => {
            ...
            return (
              <div key={id} className="flex items-center gap-3 px-4 py-3 bg-white dark:bg-[#181820] border border-black/10 dark:border-white/10 rounded-xl">
                ...
                <div className="flex-1">
                  <div className="text-xs font-medium text-[#1a1c28] dark:text-[#f2f2f8]">{session?.title || 'Chargement...'}</div>
                  <div className="text-[11px] text-[#6870a0] mt-0.5">{session?.room?.name || 'Sans salle'} · {session?.speakers?.map(s => s.fullName).join(', ') || ''}</div>
                </div>
                <button onClick={() => handleRemove(id)} className="text-[#D85A30] dark:text-[#f07060] text-base hover:scale-110 transition-transform">♥</button>
              </div>
            );
          })}
        </div>
